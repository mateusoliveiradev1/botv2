...
class TimerManager {
    ...
    static async startMatch(channel) {
        ...
        alerts.forEach(alert => {
            const timer = setTimeout(async () => {
                try {
                    ...
                    logger_1.default.error(e, `Failed to send timer alert to ${channel.name}`);
                }
            }, alert.time);
            channelTimers.push(timer);
        });
        this.timers.set(channel.id, channelTimers);
        logger_1.default.info(`Match timer started for channel ${channel.id} with ${channelTimers.length} alerts scheduled.`);
    }
    static stopMatch(channelId) {
        const existing = this.timers.get(channelId);
        if (existing) {
            existing.forEach(t => clearTimeout(t));
            this.timers.delete(channelId);
            logger_1.default.info(`Match timer stopped for channel ${channelId}.`);
            return true;
        }
        return false;
    }
