...
const manager_1 = require("../modules/tickets/manager");
    ...
    async execute(interaction) {
        ...
        if (interaction.isChatInputCommand()) {
            ...
            try {
                ...
                logger_1.default.error(error, `Error executing ${interaction.commandName}`);
                try {
                    if (interaction.replied || interaction.deferred) {
                        await interaction.followUp({ content: 'Erro ao executar comando!', flags: discord_js_1.MessageFlags.Ephemeral });
                    }
                    else {
                        await interaction.reply({ content: 'Erro ao executar comando!', flags: discord_js_1.MessageFlags.Ephemeral });
                    }
                }
                catch (e) {
        ...
        if (interaction.isButton()) {
            try {
                ...
                if (interaction.customId === 'check_mission_progress' || interaction.customId === 'refresh_mission_progress') {
                    ...
                    // Auto Claim logic
                    const claimed = await manager_2.MissionManager.claimRewards(interaction.member);
                    const embed = await manager_2.MissionManager.getProgressEmbed(interaction.member);
                    // Refresh Button
                ...
                if (interaction.customId === 'report_bug') {
                    ...
                    const descInput = new discord_js_1.TextInputBuilder()
                        ...
                        .setRequired(true);
                    modal.addComponents(new discord_js_1.ActionRowBuilder().addComponents(commandInput), new discord_js_1.ActionRowBuilder().addComponents(descInput));
                    await interaction.showModal(modal);
                }
                if (interaction.customId === 'open_ticket') {
                    await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                    const channel = await manager_1.TicketManager.createTicket(interaction.guild, interaction.user);
                ...
                if (interaction.customId === 'claim_ticket') {
                    ...
                    oldRow.components.forEach((comp) => {
                        const builder = discord_js_1.ButtonBuilder.from(comp);
                ...
                if (interaction.customId === 'close_ticket') {
                    ...
                    const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('close_ticket').setLabel('Fechando...').setStyle(discord_js_1.ButtonStyle.Danger).setDisabled(true));
                    try {
                    ...
                    setTimeout(async () => {
                        if (interaction.channel) {
                            await manager_1.TicketManager.closeTicket(interaction.channel, interaction.user);
                        }
                    }, 5000);
                }
                if (interaction.customId === 'link_account') {
                    ...
                    if (response.success && response.data) {
                        ...
                        await interaction.editReply({ embeds: [embeds_1.EmbedFactory.error('Falha na geração de credenciais.')] });
                    }
                }
                ...
                if (error.code === 10062) {
                    logger_1.default.warn(`⚠️ Unknown interaction in button handler: ${interaction.customId}`);
                }
                else {
                    logger_1.default.error(error, `Error handling button ${interaction.customId}`);
                }
            }
        }
        // 3. Select Menus (FAQ)
        if (interaction.isStringSelectMenu()) {
            ...
            if (interaction.customId === 'faq_select') {
                ...
                const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
                    ...
                    .setStyle(discord_js_1.ButtonStyle.Secondary));
                await interaction.reply({
                    embeds: [embeds_1.EmbedFactory.create('📚 Resposta FAQ', response)],
                    components: [row],
                    flags: discord_js_1.MessageFlags.Ephemeral
                });
            }
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === 'bug_report_modal') {
                ...
                await LogManager_1.LogManager.log({
                    ...
                    type: LogManager_1.LogType.ADMIN,
            ...
            if (interaction.customId === 'faq_ai_modal') {
                ...
                // Simular "pensando"
                await interaction.deferReply({ flags: discord_js_1.MessageFlags.Ephemeral });
                const result = await faq_1.faqService.search(question);
                const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder()
                    .setCustomId('ask_ai')
                ...
                if (result.found && result.answer) {
                    const embed = embeds_1.EmbedFactory.create('🤖 IA BlueZone', result.answer)
                    ...
                    const embed = embeds_1.EmbedFactory.error('Sem resposta precisa')
            ...
            if (interaction.customId === 'sitrep_modal') {
                ...
                const channel = interaction.guild?.channels.cache.find(c => c.name === '📢-sitrep');
                if (!channel) {
                    await interaction.reply({ content: '❌ Canal #sitrep não encontrado.', flags: discord_js_1.MessageFlags.Ephemeral });
                    return;
                }
                const embed = new discord_js_1.EmbedBuilder()
