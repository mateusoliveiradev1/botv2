...
export class CanvasHelper {
  ...
  static async generateRankingImage(ranking: any[], botAvatarURL?: string) {
    ...
    let logoPromise: Promise<any> = Promise.resolve(null);
    if (botAvatarURL) {
       logoPromise = loadImage(botAvatarURL).catch(() => null);
    } else {
       logoPromise = loadImage('https://wstatic-prod.pubg.com/web/live/static/og/img-og-pubg.jpg').catch(() => null);
    }
    ...
    if (headerLogo) {
      ...
      ctx.stroke();
        ...
        ctx.beginPath();
        ctx.fillStyle = '#F2A900';
        ctx.moveTo(10, 0); ctx.lineTo(70, 0); 
    ...
    for (let index = 0; index < ranking.length; index++) {
      ...
      if (index === 0) { baseColor = 'rgba(255, 215, 0, 0.2)'; glowColor = '#FFD700'; strokeColor = '#FFD700'; }
      else if (index === 1) { baseColor = 'rgba(192, 192, 192, 0.15)'; glowColor = '#C0C0C0'; strokeColor = '#C0C0C0'; }
      else if (index === 2) { baseColor = 'rgba(205, 127, 50, 0.15)'; glowColor = '#CD7F32'; strokeColor = '#CD7F32'; }
      ...
      ctx.save();
      if (index < 3) { ctx.shadowColor = glowColor; ctx.shadowBlur = 15; }
      ...
      if (index === 0) ctx.fillText('👑', x + 50, y + 65);
      else ctx.fillText(`${index + 1}`, x + 50, y + 65);
      ...
      if (entry.clan_tag) displayName = `[${entry.clan_tag}] ${displayName}`;
