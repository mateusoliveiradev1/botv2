...
export class VoiceManager {
  ...
  public static async handleInteraction(interaction: Interaction) {
    ...
    if (interaction.isButton() && interaction.customId.startsWith("voice_")) {
      ...
      if (!isOwner) {
        await interaction.reply({
          content: "⛔ Você não tem permissão para gerenciar esta sala.",
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      switch (interaction.customId) {
        case "voice_lock":
          ...
          await interaction.reply({
            content: "🔒 **Sala Trancada!** Ninguém mais pode entrar.",
            flags: MessageFlags.Ephemeral,
          });
          break;
        ...
        case "voice_unlock":
          ...
          await interaction.reply({
            content: "🔓 **Sala Destrancada!** Entrada liberada.",
            flags: MessageFlags.Ephemeral,
          });
          break;
    ...
    if (interaction.isModalSubmit()) {
      if (interaction.customId === "voice_rename_modal") {
        const newName =
          interaction.fields.getTextInputValue("voice_name_input");
        const member = interaction.member as GuildMember;
        ...
        if (
          ...
          await interaction.reply({
            content: `✅ Sala renomeada para: **${newName}**`,
            flags: MessageFlags.Ephemeral,
          });
        }
      }
      ...
      if (interaction.customId === "voice_limit_modal") {
        const limit = parseInt(
          interaction.fields.getTextInputValue("voice_limit_input"),
        );
        const member = interaction.member as GuildMember;
        ...
        if (
          ...
          await interaction.reply({
            content: `✅ Limite ajustado para: **${limit === 0 ? "Ilimitado" : limit}**`,
            flags: MessageFlags.Ephemeral,
          });
        }
      }
    ...
    if (
      ...
      if (targetMember.voice.channelId !== member.voice.channelId) {
        await interaction.reply({
          content: "❌ Esse usuário não está na sua sala.",
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      try {
        await targetMember.voice.setChannel(
          null,
          `Expulso por ${member.displayName}`,
        );
        ...
        await LogManager.log({
          ...
          fields: [
            ...
            {
              name: "Sala",
              value: member.voice.channel?.name || "Desconhecida",
              inline: true,
            },
          ],
        });
        ...
        await interaction.reply({
          content: `👢 **${targetMember.displayName}** foi removido da sala.`,
          flags: MessageFlags.Ephemeral,
        });
      } catch (e) {
        await interaction.reply({
          content: "❌ Erro ao expulsar usuário (permissões insuficientes?).",
          flags: MessageFlags.Ephemeral,
        });
      }
    }
    ...
    if (newState.channelId && newState.channel?.name === "➕ Criar Sala") {
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            `Bem-vindo ao seu canal, **${member.displayName}**.\nUse os botões abaixo para gerenciar o squad.`,
          )
          .setColor("#0099FF")
          .addFields(
            {
              name: "🔒 Privacidade",
              value: "Trancar/Destrancar",
              inline: true,
            },
    ...
    if (oldState.channelId) {
      ...
      if (
        ...
        try {
          ...
          await this.deleteState(channel.id);
      ...
      for (const channelId of this.tempChannels) {
        ...
        if (channel && channel.isVoiceBased() && channel.members.size === 0) {
          try {
            ...
            await this.deleteState(channelId);
            cleaned++;
        ...
        this.tempChannels.forEach((channelId) => {
          ...
          if (channel && channel.isVoiceBased() && channel.members.size === 0) {
            channel
              ...
              .catch((e) =>
                logger.error(
                  `Cleanup failed for ${channel.name}: ${e.message}`,
                ),
              );
            this.tempChannels.delete(channelId);
            this.deleteState(channelId);
          } else if (!channel) {
