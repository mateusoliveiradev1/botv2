...
export class LogManager {
  ...
  static async log(entry: LogEntry) {
    ...
    embed.setAuthor({
      name: `${icon} LOG DO SISTEMA | ${entry.type}`,
      iconURL: entry.guild.iconURL() || undefined,
    });
    ...
    // Footer with ID
    const logId =
      entry.footer || `LOG-ID: ${Date.now().toString(36).toUpperCase()}`;
    embed.setFooter({
      text: logId,
    ...
    try {
      // 1. Send to Discord (Async - Fire & Forget)
      channel
        .send({ embeds: [embed], files: entry.files || [] })
        .catch((e) =>
          logger.warn(`Failed to send log to Discord: ${e.message}`),
        );

      // 2. Persist to Database (AuditLog) via DatabaseManager (Mutex Protected)
      ...
      logger.error(error, "Failed to process log (DB)");
    }
  }
}
